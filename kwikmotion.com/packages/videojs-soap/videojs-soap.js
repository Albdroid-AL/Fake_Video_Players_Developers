...
(function (global, factory) {
  ...
  var rightArrow = "<svg viewBox=\"0 0 1792 1792\" xmlns=\"http://www.w3.org/2000/svg\"><path d=\"M1363 877l-742 742q-19 19-45 19t-45-19l-166-166q-19-19-19-45t19-45l531-531-531-531q-19-19-19-45t19-45L531 45q19-19 45-19t45 19l742 742q19 19 19 45t-19 45z\"/></svg>";
  ...
  var SoapMenuItem = function (_VjsMenuItem) {
    ...
    SoapMenuItem.prototype.handleClick = function handleClick(e) {
      ...
      if (this.options_.mainmenu) {
        ...
        menu.player().getChild('ControlBar').getChild('soap').createMenu();
        ...
        var options = {
          name: 'seasonstitle',
          'label': '<span class="left-arrow">' + rightArrow + '</span>' + menu.options_.title + ' <span class="addon-title">' + this.options_.controlText + '</span>',
          className: 'vjs-seasons-title',
          'selectable': false,
          mainmenu: true
        };
        var menuItem = new SoapMenuItem(menu.player(), options);
        menu.addItem(menuItem);
        if (this.options_.episodes && this.options_.episodes.length) {
          var _loop = function _loop(idx) {
            var uniqId = 'episode_' + _this2.options_.episodes[idx].season + '_' + idx;
            var uniqName = 'episode_' + _this2.options_.episodes[idx].season;
            options = {
              'label': '<input type="radio" name="' + uniqName + '" id="' + uniqId + '"><label for="' + uniqId + '" class="vjs-episode-title">' + _this2.options_.episodes[idx].title + '<span class="up-arrow">' + rightArrow + '</span></label><a class="vjs-episode-description" target="_blank" href="' + _this2.options_.episodes[idx].url + '"><img src="' + _this2.options_.episodes[idx].image + '"/>' + _this2.options_.episodes[idx].description + '</a>',
              'selectable': true,
              className: 'vjs-episode-item'
            };
            var newmenuItem = new SoapMenuItem(menu.player(), options);
            (function (url) {
              newmenuItem.el().querySelector('.vjs-episode-description').addEventListener('touchstart', function () {
                window.top.location.href = url;
              });
            })(_this2.options_.episodes[idx].url);
            menu.addItem(newmenuItem);
          };

          for (var idx = 0; idx < this.options_.episodes.length; idx++) {
            _loop(idx);
          }
        }
        menu.lockShowing();
  ...
  var SoapButton = function (_VjsButton) {
    ...
    function SoapButton(player, options) {
      ...
      _this.el().querySelector('button').insertAdjacentHTML('afterbegin', '<div class="vjs-soap-ico">' + btnIco + '</div>');
      return _this;
    }
    ...
    SoapButton.prototype.createMenu = function createMenu() {
      ...
      if (this.options_.title && this.options_.title != '') {
        options = {
          name: 'seasonstitle',
          'label': '<span class="left-arrow">' + rightArrow + '</span>' + this.options_.title + ' <span class="addon-title"></span>',
          className: 'vjs-seasons-title',
          'selectable': false,
          mainmenu: true
        };
      ...
      while (season) {
        if (season.nodeType === 1 && season.nodeName && season.nodeName.match(/(\d+)/i)) {
          ...
          while (episode) {
            if (episode.nodeName && episode.nodeName.match(/^episode(\d+)/i)) {
              ...
              for (var nidx = 0; nidx < nodes.length; nidx++) {
                epObj[nodes[nidx].nodeName] = nodes[nidx].innerHTML;
              }
              episodes.push(epObj);
            }
          ...
          options = {
            'label': '<span>' + seasonName + '</span><span class="right-arrow">' + rightArrow + '</span>',
            'selectable': true,
            'episodes': episodes,
    ...
    function Soap(player, options) {
      ...
      _this.buildMenu = function (error, response, responseBody) {
        ...
        if (responseBody && responseBody.childNodes && responseBody.childNodes.length) {
          var titleNode = responseBody.evaluate('//title[1]', responseBody, null, XPathResult.FIRST_ORDERED_NODE_TYPE, null);
          var seasonsNode = responseBody.evaluate('//playlist[1]/seasons[1]', responseBody, null, XPathResult.FIRST_ORDERED_NODE_TYPE, null);
          if (seasonsNode.singleNodeValue) {
            soapButton = new SoapButton(player, {
              name: 'soap',
              title: titleNode ? titleNode.singleNodeValue.innerHTML : '',
