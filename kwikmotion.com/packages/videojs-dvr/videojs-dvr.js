 ...
 */
var getPlayerRange = function getPlayerRange(player) {
  return player.tech_ && player.tech_.shakaPlayer ? player.tech_.shakaPlayer.seekRange() : {
...
var onPlayerReady = function onPlayerReady(player, options) {
  ...
  var Slider = player.controlBar.progressControl.seekBar;
  var mouseTimeDisplay = Slider.getChild('mouseTimeDisplay');
  var playProgressBar = Slider.getChild('playProgressBar');
  var loadProgressBar = Slider.getChild('loadProgressBar');
  ...
  Slider.handleMouseMove = function (event) {
    ...
    var newTime2 = getPlayerRange(this.player_).end - calculate * customTime;
  ...
  if (mouseTimeDisplay) {
    mouseTimeDisplay.update = function update(seekBarRect, seekBarPoint) {
      ...
      this.rafId_ = this.requestAnimationFrame(function () {
        ...
        if (seekBarPoint !== 0 && _this.player_.duration() !== Number.POSITIVE_INFINITY) {
          _this.getChild('timeTooltip').update(seekBarRect, seekBarPoint, '-' + content2);
        }
      });
    };
  }
...
var onTimeUpdate = function onTimeUpdate(player, e) {
  ...
  var time = getPlayerRange(player);

  if (!time || !time.end) {
    return;
  }

  if (time.end - player.currentTime() < 20) {
    player.controlBar.liveButton.addClass('onair');
  } else {
    player.controlBar.liveButton.removeClass('onair');
  }
  player.duration(getPlayerRange(player).end - getPlayerRange(player).start);
  player.getChild('ControlBar').getChild('RemainingTimeDisplay').updateContent();
  if (!timeLive) {
    timeLive = customTime = getPlayerRange(player).end - getPlayerRange(player).start;
  }
};
...
var LiveButton = function (_Button) {
  ...
  LiveButton.prototype.handleClick = function handleClick(event) {
    var currentTime = getPlayerRange(this.player_).end;
